...
public class FoliaSuppressor implements ISuppressor {

    private final Method getHandle;
    private final Method level;
    private final Method getCurrentWorldData;
    private final Field neighborUpdater;
    private final Field update;
    public FoliaSuppressor() throws ClassNotFoundException, NoSuchMethodException, NoSuchFieldException {
        Class<?> clazz;
        ...
        getCurrentWorldData = clazz.getMethod("getCurrentWorldData");
        getCurrentWorldData.setAccessible(true);

        clazz = Class.forName("io.papermc.paper.threadedregions.RegionizedWorldData");
    ...
    public void Suppress(Player p, BiConsumer<Field, Object> consumer) {
        try {
            ...
            Object worldData = getCurrentWorldData.invoke(serverLevel);
            Object updater = neighborUpdater.get(worldData);
